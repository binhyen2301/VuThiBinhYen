...
export class UsersComponent implements OnInit {
    ...
    khoaTaiKhoan(id: any) {
        this.blockedPanel = true;
        this.usersService.khoaTaiKhoan(id).subscribe(
            () => {
    ...
    moTaiKhoan(id: any) {
        this.blockedPanel = true;
        this.usersService.moTaiKhoan(id).subscribe(
            () => {
                this.notificationService.showSuccess(
        ...
        if (this.selectedItems != null && this.selectedItems.length > 0) {
            ...
            this.usersService.getUserRoles(userId).subscribe(
                (response: any) => {
                    ...
                    if (
                        this.selectedRoleItems.length === 0 &&
    ...
    deleteRolesConfirm(roleNames) {
        this.blockedPanelRole = true;
        this.usersService
            .removeRolesFromUser(this.selectedItems[0].id, roleNames)
            .subscribe(
                () => {
                    this.loadUserRoles();
                    this.selectedRoleItems = [];
